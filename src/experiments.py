...
def run_experiment_bio_1(
    ...
    if constants is None:
        constants = ['const']
    rs_equation_generator.generate_pretrain_dataset(train_size, 14, 'train', functions=functions,
                                                    all_tokens=functions + free_variables + constants)
    rs_equation_generator.generate_pretrain_dataset(test_size, 14, 'val', functions=functions,
                                                    all_tokens=functions + free_variables + constants)
    with open('wandb_key') as f:
        os.environ["WANDB_API_KEY"] = f.read().strip()
